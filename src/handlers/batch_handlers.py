...
async def add_batches(
    batches_in: List[BatchModelInRus],
...
@router.get(
    "/{batch_id}",
    responses={404: {"model": Message, "description": "Batch was not found"}},
)
async def get_batch(
    batch_id: int, session: AsyncSession = Depends(get_async_session)
) -> BatchModelOutWithProducts:
    batch_service = BatchService(session)
...
@router.patch(
    ...
    responses={
        ...
        500: {
            "model": Message,
            "description": "Error updating. Unique number and unique date already exists",
        },
    },
)
async def update_batch(
    ...
    if not updated_batch:
        raise HTTPException(
            status_code=500,
            detail="Error updating. Unique number and unique date already exists",
        )
...
async def get_batches_with_filters(
    filters: BatchModelInFilters = Depends(BatchModelInFilters),
    session: AsyncSession = Depends(get_async_session),
) -> List[BatchModelOut]:
    batch_service = BatchService(session)
    filtered_batches = await batch_service.filter_batches(filters)
    return [
        BatchModelOut.from_orm(filtered_batch) for filtered_batch in filtered_batches
    ]
