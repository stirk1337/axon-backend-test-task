...
from src.handlers.pydantic_models.product_model import (
    ProductInRus,
    ProductInAggregate,
    ProductOutAggregate,
)
...
async def add_product(
    products_in: List[ProductInRus], session: AsyncSession = Depends(get_async_session)
):
    batch_service = BatchService(session)
    product_service = ProductService(session)
    for product in products_in:
        batch = await batch_service.get_batch_by_number_and_date(
            product.number, product.batch_date
        )
        if batch:
...
@router.patch(
    ...
    responses={
        ...
        400: {
            "model": Message,
            "description": "unique code is attached to another batch or already used",
        },
    },
)
async def aggregate_product(
    code: str,
    product_in: ProductInAggregate,
